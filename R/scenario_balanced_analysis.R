# Analysis of Random effects from simulation 02_three_varying_parameters 

# 02/09/2022
# 21/09/2022 finalized 
...
dir_scenario <- "../../github-repo-data/balanced_scenario/"
...
track_matrix <- read.csv(paste0(dir_scenario,"track_matrix.csv")) %>% 
  select(-X)
...
# NA values ID
sd_id_nan <- sd_id %>% 
  group_by(sd_spatial, sd_practice, n_practices) %>% 
  summarize(n_nan = sum(is.na(mean))) %>% 
  ggplot(mapping = aes(x=as.factor(sd_practice), y = n_nan))+
  geom_jitter(width=.2,height=0,alpha=.6,aes(size=n_nan,color=as.factor(n_practices)))+
...
### Practice ----
ggplot(data = sd_pr, aes(x=as.factor(sd_spatial), y = mean, color=as.factor(n_practices)))+
  geom_boxplot()+
...
# Can we read all files at once? -> takes like 20' 
# setwd(paste0("/Users/u0121893/Library/CloudStorage/OneDrive-KULeuven/phd/research/",
#              "simulation/spatial/hpc_wf/pc_prior_balanced_scenario/",
...
ggplot(data = sp_test2, aes(x=as.factor(sd_practice), y = mae, color=as.factor(n_practices)))+
  geom_boxplot()+
...
setwd(paste0("/Users/u0121893/Library/CloudStorage/OneDrive-KULeuven/phd/projects/",
             "simulation/spatial/github-repo-data/balanced_scenario/",
